...
status: active
updated: 2026-05-08
tags: [learning, project, apollo, outlook, addin, diary]
related:
...
- ✅ Phase 12: Body-push consistency fix (2026-05-08) — three-part fix shipped over the session: (a) read body fresh at click time so Nick's typed content is captured, (b) when Apollo returns multiple emailer_messages, pick the newest by `created_at` so we don't update orphan messages from previous test pushes, (c) use Apollo's actual field values `outreach_manual_email` + `drafted` in the candidate filter so the picker is deterministic. Verified by Nick on two consecutive live pushes after deploy.
...
- 2026-05-08: Body-push reliability session (`v=20260508a` → `v=20260508d`). Started from "more times than not it doesn't carry over." Three independent bugs surfaced in sequence and were each diagnosed via the in-add-in console logs Nick paste back. End state: two consecutive live pushes landed correctly with different recipients, the `[apollo] picked message` log line is now informative when the filter does/doesn't find a candidate, and the picker is deterministic rather than relying on the newest-by-date fallback. Cache-bust ended at `v=20260508d`.

## Current Cache-Bust Version

`v=20260508d` — bump the trailing letter on every code change so Office Add-in iframes pick up the new code instead of serving cached versions.

## Apollo emailer_message field values — corrected (2026-05-08)

Confirmed from Nick's production logs on a real push, NOT inferred:

- **`type`** = `"outreach_manual_email"` (not `"manual_email"` as assumed)
- **`status`** = `"drafted"` (not `"queued"`, `"pending"`, or `"draft"`)
- A freshly-enrolled step 1 has `current body length: 63` (the empty template)
- The MongoDB ObjectId in `id` and `created_at` are both populated and lex/ISO-sortable

The filter in `tryUpdateManualMessageBody` uses both the real values plus the older guesses for forward-compat across Apollo accounts/versions. Status filter also includes `unscheduled` as a defensive add for sequences where step 1 hasn't been activated.

If a future Apollo change breaks this again, the diagnostic log line `[apollo] picked message id=... status=... type=... (N manual candidate(s) of M total)` will surface the new values immediately — Nick can paste them and we update the filter strings.

## Stale emailer_messages — silent failure mode (2026-05-08)

Apollo's `/emailer_messages/search` filtered by `contact_ids + emailer_campaign_ids` returns ALL messages ever created for that contact in that sequence — including stale ones from previous enrollments where the contact was added, removed, then re-added (which is exactly what Nick's testing flow does).

Symptom: PUT body succeeds, verify GET succeeds, but Apollo's UI step 1 still shows the wrong/old content. Reason: we PUT to a stale orphan message, not the one Apollo will actually send. Diagnosed by Nick on 2026-05-08 — search returned **5 messages** for a 3-step sequence with one contact, and the targeted message had `current body length: 25213` (suspiciously close to the previously-pushed length, not a fresh template).

**Fix (in `apollo.js` `tryUpdateManualMessageBody`):** filter to manual_email + non-terminal status, then sort by `created_at` desc (with ObjectId lex fallback — MongoDB ObjectIds encode timestamp in the first 8 hex chars), pick the newest. Logs `[apollo] picked message id=... (N manual candidate(s) of M total)` so future runs surface the same problem class faster.

**Operational note:** when Nick removes a test contact from a sequence to re-test, the underlying emailer_messages don't get deleted — they remain queued/orphaned and pile up. Currently harmless because of the newest-wins picker, but if pile-up ever causes issues, an Apollo support ask or a one-time admin sweep could clean them out.

## Body-read at click time, not pane-load time (2026-05-08)

Original bug: `draftContext.bodyHtml` was captured during `loadEverything()` at pane-open. Nick's typed reply, written AFTER the pane loaded, never made it into the push.

**Fix (in `taskpane.js` `handlePush`):** read body fresh via `Office.context.mailbox.item.body.getAsync(Office.CoercionType.Html, ...)` AND keep the cached one. Pick whichever has more content (defensive: some Office.js / Outlook combos returned empty/stripped on second-read in early test versions, so cached wins as fallback). Logs both lengths so the diagnostic surface is obvious.
